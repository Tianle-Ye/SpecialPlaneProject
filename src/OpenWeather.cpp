...
void MyOpenWeather::get_wind_info(double lat, double lon){
    ...
    if(curl && !DImplementation->my_api.empty()){
        ...
        if (res == CURLE_OK) {
            ...
            DImplementation->current_wind_info.speed = data["wind"]["speed"];
            DImplementation->current_wind_info.deg = data["wind"]["deg"];
            DImplementation->current_wind_info.state = true;
        }
...
WindInfo MyOpenWeather::get_wind() const{
    return DImplementation->current_wind_info;
}